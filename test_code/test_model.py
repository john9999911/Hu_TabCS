...
class CodeSearcher:
    def __init__(self, conf=None):
        ...
        self.vocab_methname = load_pickle(self.path + self.data_params['vocab_methname'])
        self.vocab_apiseq = load_pickle(self.path + self.data_params['vocab_apiseq'])
        self.vocab_sbt = load_pickle(self.path + self.data_params['vocab_sbt'])
        self.vocab_tokens = load_pickle(self.path + self.data_params['vocab_tokens'])
        self.vocab_desc = load_pickle(self.path + self.data_params['vocab_desc'])
    ...
    def search(self, model, query, n_results=10):
        ...
        padded_methnames = pad(methnames, self.data_params['methname_len'])
        padded_apiseqs = pad(apiseqs, self.data_params['apiseq_len'])
        padded_tokens = pad(tokens, self.data_params['tokens_len'])
        padded_sbt = pad(sbt, self.data_params['sbt_len'])

        data_len = len(tokens)
        desc = convert(self.vocab_desc, query)  # convert desc sentence to word indices
        padded_desc = pad([desc] * data_len, self.data_params['desc_len'])
